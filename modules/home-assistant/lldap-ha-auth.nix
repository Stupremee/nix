{writeShellScriptBin, gnused, curl, jq, ...}: writeShellScriptBin "lldap-ha-auth" ''
#!/usr/bin/env bash
...
# Check username and password are present and not malformed.
  if [ -z "$username" ] || [ -z "$password" ]; then
    ...
    username_match=$(echo "$username" | ${gnused}/bin/sed -r "s/$USERNAME_PATTERN/x/")
  ...
  RESPONSE=$(${curl}/bin/curl -f -s -X POST -m "$TIMEOUT" -H "Content-type: application/json" -d '{"username":"'"$username"'","password":"'"$password"'"}' "$SERVER_URL/auth/simple/login")
  if [[ $? -ne 0 ]]; then
    log "Auth failed"
    exit 1
  fi
  TOKEN=$(${jq}/bin/jq -e -r .token <<<$RESPONSE)
  if [[ $? -ne 0 ]]; then
    log "Failed to parse token"
    exit 1
  fi

  RESPONSE=$(${curl}/bin/curl -f -s -m "$TIMEOUT" -H "Content-type: application/json" -H "Authorization: Bearer $TOKEN" -d '{"variables":{"id":"'"$username"'"},"query":"query($id:String!){user(userId:$id){displayName groups{displayName}}}"}' "$SERVER_URL/api/graphql")
  if [[ $? -ne 0 ]]; then
    log "Failed to get user"
    exit 1
  fi

  USER_JSON=$(${jq}/bin/jq -e .data.user <<<$RESPONSE)
  if [[ $? -ne 0 ]]; then
    log "Failed to parse user json"
    exit 1
  fi

  if [[ -n $2 ]] && ! ${jq}/bin/jq -e '.groups|map(.displayName)|index("'"$2"'")' <<<$USER_JSON >/dev/null 2>&1; then
    log "User is not in group '$2'"
    exit 1
  fi

  DISPLAY_NAME=$(${jq}/bin/jq -r .displayName <<<$USER_JSON)

  IS_ADMIN=false
  if [[ -n $3 ]] && ${jq}/bin/jq -e '.groups|map(.displayName)|index("'"$3"'")' <<<"$USER_JSON" >/dev/null 2>&1; then
    IS_ADMIN=true
  fi

  IS_LOCAL=false
  if [[ -n $4 ]] && ${jq}/bin/jq -e '.groups|map(.displayName)|index("'"$4"'")' <<<"$USER_JSON" >/dev/null 2>&1; then
    IS_LOCAL=true
  fi
